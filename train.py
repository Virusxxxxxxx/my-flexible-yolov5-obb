...
from utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    ...
    check_requirements, print_mutation, set_logging, one_cycle, colorstr, write_target_count
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    with torch_distributed_zero_first(rank):
        check_dataset(data_dict)  # Download dataset if not found locally
    ...
    compute_loss = ComputeLoss(model, csl_label_flag=True)  # init loss class
    ...
    # obb
    # AP结果写入文件
    with open(save_dir / 'classAP.csv', 'w', encoding='utf-8', newline="") as f_ap:
        csv_ap = csv.writer(f_ap)
        csv_ap.writerow(['Epoch', 'mAP', *classnames])

    # 记录每个epoch的target个数
    with open(save_dir / 'target_count.csv', 'w', encoding='utf-8', newline="") as f_target:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(5, device=device)  # obb
        if rank != -1:
        ...
        write_target_count(targets_counts, save_dir)
