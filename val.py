# --------------------------------------------------------
"""  在线评估
1、merge检测结果：将源路径中所有的txt目标信息,经nms后存入目标路径中的同名txt
2、按照类别分类检测结果：将result_merged_path文件夹中的所有txt中的目标提取出来,按照目标类别分别存入 Task1_类别名.txt中
3、生成校验数据集名称文件：将srcpath文件夹下的所有子文件名称打印到imgnamefile.txt中
"""
# --------------------------------------------------------
import csv
...
from utils.plots import plotChart_interest, plotChart
...
def val(detectionPath, rawImagePath, rawLabelPath, resultPath):
    ...
    # 生成校验数据集名称文件
    # 将srcpath文件夹下的所有子文件名称打印到imgnamefile.txt中
    image2txt(
    ...
    if skippedClassCount != len(classnames):
        ...
        # recall = recall / (len(classnames) - skippedClassCount)
        # precision = precision / (len(classnames) - skippedClassCount)
    else:
        map = 0
        # recall = 0
        # precision = 0
