...
from utils.general import bbox_iou, rbox_iou
...
def gaussian_label(label, num_class, u=0, sig=4.0):  # obb
    """
        转换成CSL Labels：
...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, csl_label_flag=True):
        ...
        if g > 0:
            ...
            BCEangle = FocalLoss(BCEangle, g)  # obb
        ...
        # 根据det.nl即检测头数(3 4 5)，确定对应输出的损失系数
        ...
        self.BCEangle = BCEangle  # obb
        for k in 'na', 'nc', 'nl', 'anchors':
            """
            na: number of anchors 6//2=3  每种网络3种anchor框
            nc: number of classes
            nl: number of detection layers  3  三种步长的检测网络
            anchors: anchors size
            """
            setattr(self, k, getattr(det, k))
        self.csl_label_flag = csl_label_flag

    def __call__(self, p, targets):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        langle = torch.zeros(1, device=device)  # obb
        # tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets
        tcls, tbox, indices, anchors, tangle = self.build_targets(p, targets)  # obb
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # box Regression
                pxy = ps[:, :2].sigmoid() * 2. - 0.5  # 对前向传播结果xy进行回归
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]  # 对前向传播结果wh进行回归
                pbox = torch.cat((pxy, pwh), 1).to(device)  # predicted box
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # iou(prediction, target)  !!CIoU!!
                lbox += (1.0 - iou).mean()  # iou loss

                # Objectness
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * iou.detach().clamp(0).type(tobj.dtype)  # iou ratio

                # Objectness plan 2 置信度分支的权重系数为《旋转框IOU》  -> but slow
                ...
                # tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * riou.detach().clamp(0).type(tobj.dtype)
                ...
                # angle 损失 ############################ obb ###########################
                if not self.csl_label_flag:
                    ttheta = torch.full_like(ps[:, class_index:], self.cn, device=device)
                    ttheta[range(n), tangle[i]] = self.cp
                    langle += self.BCEangle(ps[:, class_index:], ttheta)  # BCE Θ类别损失以BCEWithLogitsLoss来计算
                else:
                    ...
                    langle += self.BCEangle(ps[:, class_index:], ttheta)
                # ################################ obb ##################################
        ...
        s = 3 / len(p)  # obb
        lbox *= self.hyp['box'] * s  # obb
        lobj *= self.hyp['obj'] * s * (1.4 if len(p) == 4 else 1.)  # obb
        lcls *= self.hyp['cls'] * s  # obb
        langle *= self.hyp['angle'] * s  # obb
        ...
        loss = lbox + lobj + lcls + langle  # obb
    ...
    def build_targets(self, p, targets):
        ...
        gain = torch.ones(8, device=targets.device)  # obb
        ...
        for i in range(self.nl):
            ...
            tcls.append(c)  # class
            tangle.append(angle)  # obb

        # return tcls, tbox, indices, anch
        return tcls, tbox, indices, anch, tangle  # obb
